...
class Util:
    ...
    Methods:
        ...
        is_senin_kamis() -> bool:
            Checks if the current weekday is Monday to Thursday.

        is_jumat_sabtu() -> bool:
            Checks if the current weekday is Friday or Saturday.
    ...
    Example:
        ...
        is_senin_kamis = util.is_senin_kamis()
        is_jumat_sabtu = util.is_jumat_sabtu()
    ...
    def get_jurnal(self) -> dict:
        ...
        return {
            ...
            "Hari Kerja 3": [
                ...
                {
                    "kegiatan": value[10][1],
                    "jam_mulai": get_sheet_time(value[10][2]),
                    "menit_mulai": get_sheet_time(value[10][3]),
                    "jam_selesai": get_sheet_time(value[10][4]),
                    "menit_selesai": get_sheet_time(value[10][5]),
                    "skp": get_skp_value(value[10][6])[0],
                    "skp_value": get_skp_value(value[10][6])[1],
                    "jumlah_diselesaikan": int(value[10][7])
                },
                {
                    "kegiatan": value[11][1],
                    "jam_mulai": get_sheet_time(value[11][2]),
                    "menit_mulai": get_sheet_time(value[11][3]),
                    "jam_selesai": get_sheet_time(value[11][4]),
                    "menit_selesai": get_sheet_time(value[11][5]),
                    "skp": get_skp_value(value[11][6])[0],
                    "skp_value": get_skp_value(value[11][6])[1],
                    "jumlah_diselesaikan": int(value[11][7])
                },
                {
                    "kegiatan": value[12][1],
                    "jam_mulai": get_sheet_time(value[12][2]),
                    "menit_mulai": get_sheet_time(value[12][3]),
                    "jam_selesai": get_sheet_time(value[12][4]),
                    "menit_selesai": get_sheet_time(value[12][5]),
                    "skp": get_skp_value(value[12][6])[0],
                    "skp_value": get_skp_value(value[12][6])[1],
                    "jumlah_diselesaikan": int(value[12][7])
                }
            ]
        }

    def rentang_waktu_kerja(self) -> list:
        """
        Calculates the working time range.

        This method calculates the working time range based on the specified start and end dates. It iterates through each day in the range and determines whether it is a working day or a holiday. If it is a working day, it assigns a status of "Hari Kerja 1", "Hari Kerja 2", or "Hari Kerja 3" based on the number of working days already counted. If it is a holiday, it assigns a status of "Hari Libur". The method returns a list of tuples, where each tuple contains the date and the corresponding status.

        Returns:
            list: A list of tuples containing the date and the corresponding status.

        Example:
            >>> util = Util()
            >>> rentang = util.rentang_waktu_kerja()
            >>> print(rentang)
            [
                (datetime.date(2022, 1, 1), "Hari Libur"),
                (datetime.date(2022, 1, 2), "Hari Kerja 1"),
                (datetime.date(2022, 1, 3), "Hari Kerja 2"),
                ...
            ]
        """
        # Tanggal awal dan akhir rentang yang Anda inginkan
        tanggal_mulai = datetime.strptime(
            os.environ['waktu_mulai'], "%Y-%m-%d").date()
        tanggal_selesai = datetime.strptime(
            os.environ['waktu_selesai'], "%Y-%m-%d").date()

        # Inisialisasi daftar untuk menyimpan hasil
        hasil = []
        # Hitung selisih hari antara tanggal awal dan akhir
        selisih_hari = (tanggal_selesai - tanggal_mulai).days
        # Inisialisasi penghitung untuk menghitung hari aktif
        hitung_hari_aktif = 0

        # Loop melalui setiap hari dalam rentang tanggal
        for i in range(selisih_hari + 1):
            tanggal_sekarang = tanggal_mulai + timedelta(days=i)
            # Cek apakah hari ini adalah hari aktif (3 hari aktif, 1 hari libur)
            if hitung_hari_aktif < 3:
                if hitung_hari_aktif == 0:
                    status = "Hari Kerja 1"
                elif hitung_hari_aktif == 1:
                    status = "Hari Kerja 2"
                elif hitung_hari_aktif == 2:
                    status = "Hari Kerja 3"
                hasil.append((tanggal_sekarang, status))
                hitung_hari_aktif += 1
            else:
                hasil.append((tanggal_sekarang, "Hari Libur"))
                hitung_hari_aktif = 0
        return hasil

    def status_hari_ini(self):
        """
        Checks the status for the current day.

        This method iterates through the date and status pairs returned by the 'rentang_waktu_kerja' method. It compares each date with the current date in the 'Asia/Jakarta' timezone. If a match is found, the corresponding status is returned.

        Returns:
            str: The status for the current day.

        Example:
            >>> util = Util()
            >>> util.status_hari_ini()
            'Hari Kerja 1'
            >>> util.status_hari_ini()
            'Hari Kerja 2'
        """
        for tanggal, status in self.rentang_waktu_kerja():
            if tanggal == datetime.now(pytz.timezone('Asia/Jakarta')).date():
                return status
